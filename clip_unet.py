...
class ResidualBlock(Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, time_channels: int, text_channels: int = 512,
        ...
        * `dropout` is the dropout rate
        ...
        # Linear layer for text embeddings
        self.text_emb = nn.Linear(text_channels, out_channels)
        self.text_act = Swish()

        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor, t: torch.Tensor, te: torch.Tensor):
        """
        ...
        * `t` has shape `[batch_size, time_channels]`
        ...
        # Add text embeddings
        h += self.text_emb(self.text_act(te))[:, :, None, None]
...
class AttentionBlock(Module):
    ...
    def forward(self, x: torch.Tensor, t: Optional[torch.Tensor] = None, te: Optional[torch.Tensor] = None):
        """
        * `x` has shape `[batch_size, in_channels, height, width]`      
        * `t` has shape `[batch_size, time_channels]`
        """
